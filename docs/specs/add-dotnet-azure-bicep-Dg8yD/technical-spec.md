...
| `starter-verify` skill entry point | `.claude/skills/starter-verify/` (referenced in `CLAUDE.md`) | The `/ship` flow invokes this skill to verify the clean-boot contract on each starter. The .NET starter's clean-boot commands (`dotnet build`, `dotnet test`, `dotnet format --verify-no-changes`) are registered in this skill's configuration from Phase 1. |
...
| **Phase 1 — shipped (#103)** | `starters/dotnet-azure/` | Project skeleton, `GET /health` minimal API, `ApiResponse<T>` + non-generic factory helpers, xUnit + `WebApplicationFactory<Program>` tests, starter CLAUDE.md, `/check` slash command, `starter-verify` dotnet target, `dotnet-starter-check.yml` CI workflow. |
| **Phase 2 — shipped (#106)** | `starters/dotnet-azure/infra/`, `.github/workflows/dotnet-azure-*.yml` | Bicep orchestrator + 6 child modules (network/identity/observability/data-postgres/data-azuresql/compute), 3 parameter examples, bicep-validate CI, workflow_dispatch deploy with OIDC. `dataProvider` parameter selects Postgres or Azure SQL. |
