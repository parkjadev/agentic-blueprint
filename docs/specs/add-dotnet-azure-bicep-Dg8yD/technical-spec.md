...
| **Phase 1 — shipped (#103)** | `starters/dotnet-azure/` | Project skeleton, `GET /health` minimal API, `ApiResponse<T>` + non-generic factory helpers, xUnit + `WebApplicationFactory<Program>` tests, starter CLAUDE.md, `/check` slash command, `starter-verify` dotnet target, `dotnet-starter-check.yml` CI workflow. Phase 2 builds on top. |
