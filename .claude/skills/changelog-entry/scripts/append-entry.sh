#!/usr/bin/env bash
# Append a keepachangelog entry under ## [Unreleased] in CHANGELOG.md.
# Usage: append-entry.sh --category <name> --message "..." --pr <n>
...
if [[ -z "$category" || -z "$message" || -z "$pr" ]]; then
  echo "usage: append-entry.sh --category <Added|Changed|Deprecated|Removed|Fixed|Security> --message \"...\" --pr <n>" >&2
  exit 2
fi
...
# Locate the [Unreleased] block. Create the category subheader if missing.
...
if not unreleased:
    # Insert a new Unreleased block near the top (after first H1 heading).
    insert = "## [Unreleased]\n\n### " + category + "\n\n" + entry + "\n\n"
...
# End of the Unreleased block = next line matching ^## \[
...
if m:
    # Append entry at end of this category section.
    cat_start = m.end()
    ...
    # Add a new category subheader at the end of the Unreleased block.
