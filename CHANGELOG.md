...
- **.NET + Azure starter Phase 2** — Bicep infrastructure under `starters/dotnet-azure/infra/`: `main.bicep` orchestrator plus `network`, `identity`, `observability`, `data` (Postgres), `data-azuresql`, and `compute` modules. VNET-integrated Container Apps, Entra-only managed-identity auth, `dataProvider` parameter selects Postgres or Azure SQL in-tree. Ships `dev`/`staging`/`prod` `.bicepparam.example` placeholders and `.env.example`; real values stay gitignored.
- `.github/workflows/dotnet-azure-bicep-validate.yml` — path-scoped CI workflow that runs `bicep build` over `main.bicep`, every child module, and every parameter example on any PR that touches `starters/dotnet-azure/infra/**`.
- `.github/workflows/dotnet-azure-deploy.yml` — adopter-facing `workflow_dispatch` deploy workflow with OIDC federation, `what-if` → `sub create` → `/health` smoke test. Requires adopter to configure `AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`, and `BICEPPARAM_CONTENT_<env>` secrets.
